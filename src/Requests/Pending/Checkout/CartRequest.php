<?php declare(strict_types=1);

namespace Zonos\ZonosSdk\Requests\Pending\Checkout;

use Zonos\ZonosSdk\Connectors\Checkout\ZonosConnector;
use Zonos\ZonosSdk\Data\Checkout\Cart;
use Zonos\ZonosSdk\Requests\ZonosRequest;
use Zonos\ZonosSdk\Responses\Queries\Checkout\CartQueryResponse;
use Zonos\ZonosSdk\Utils\GqlBuilder;

class CartRequest extends PendingZonosRequest
{
  private bool $withRetry;
  protected const DEFAULT_ATTRIBUTES = [
    'id',
  ];

  public function __construct(ZonosConnector $connector, public array $args = [], bool $withRetry)
  {
    parent::__construct($connector, GqlBuilder::make('query', 'cart', $args));
    $this->withRetry = $withRetry;
  }

  public function get(string ...$fields): ?Cart
  {
    return $this->response(...$fields)->resolve();
  }

  public function response(string ...$fields): CartQueryResponse
  {
    $query = $this->query->withFields($this->normalizeFields($fields));
    $request = new ZonosRequest(CartQueryResponse::class, (string)$query);
    $response = $this->connector->send($request)->throw();

    if ($response->json('data.cart') === null && $this->withRetry) {
      $request->headers()->add('credentialToken', $this->connector->getTestCredentialToken());
      $response = $this->connector->send($request)->throw();
    }

    assert($response instanceof CartQueryResponse);

    return $response;
  }
}
